...
/-!
# Proof of Aczél's Smoothness Theorem

Any continuous solution H : ℝ → ℝ of the d'Alembert functional equation
  H(t + u) + H(t - u) = 2 · H(t) · H(u)
with H(0) = 1 is real analytic (ContDiff ℝ ⊤).

The proof:
1. **Integration bootstrap**: Continuous → C^∞ via an antiderivative representation.
2. **ODE derivation**: C^∞ + d'Alembert ⟹ H'' = c·H for c = H''(0).
3. **Classification**: Identify H as cosh(λ·), cos(λ·), or 1; conclude analyticity.

Reference: Aczél, "Lectures on Functional Equations" (1966), Chapter 3.
-/

namespace IndisputableMonolith.Cost.FunctionalEquation
...
/-! ## Phase 1: Integration Bootstrap (Continuous → C^∞) -/
...
private lemma representation_formula (H : ℝ → ℝ) (h_cont : Continuous H)
  ...
  -- Strategy: avoid integral substitution by using FTC + is_const_of_deriv_eq_zero.
  -- We prove ∫₀ᵈ H(t+u)du = Phi(t+d)-Phi(t) and ∫₀ᵈ H(t-u)du = Phi(t)-Phi(t-d)
  -- by showing both sides have the same derivative (w.r.t. d) and agree at d=0.
  -- Then the d'Alembert equation integrated gives the representation.
  ...
  -- Step 1: ∫₀ᵈ H(t+u) du = Phi(t+d) - Phi(t)
  -- Proof: define F(d) = LHS - RHS, show F'=0 and F(0)=0, so F=0.
  ...
  -- Step 2: ∫₀ᵈ H(t-u) du = Phi(t) - Phi(t-d)
...
/-! ## Phase 2: General ODE Derivation -/
...
/-! ## Phase 3: Classification and Analyticity -/
...
private theorem dAlembert_contDiff_top (H : ℝ → ℝ)
    ...
    (h_dAl : ∀ t u, H (t + u) + H (t - u) = 2 * H t * H u) :
    ContDiff ℝ ⊤ H := by
  rcases dAlembert_classification H h_one h_cont h_dAl with h | ⟨α, h⟩ | ⟨α, h⟩
  · rw [show H = fun _ => (1 : ℝ) from funext h]
    exact contDiff_const
  · rw [show H = fun t => Real.cosh (α * t) from funext h]
    exact Real.contDiff_cosh.comp (contDiff_const.mul contDiff_id)
  · rw [show H = fun t => Real.cos (α * t) from funext h]
    exact Real.contDiff_cos.comp (contDiff_const.mul contDiff_id)

/-! ## Instance -/

instance : AczelSmoothnessPackage where
  smooth_of_dAlembert H h_one h_cont h_dAl := dAlembert_contDiff_top H h_one h_cont h_dAl
